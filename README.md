...
- 若上述設定後仍無法執行：
  - 一般 > 勾選「以最高權限執行」
  - 設定 > 「如果工作已經在執行，則套用下列規則」> 下拉式選單改為 「停止現有的執行個體」或「平行執行新執行個體」(建議勾選 「如果工作執行時間超過以下時間即停止」（例如設定為 1 小時或 2 小時），強制清理卡死的爬蟲。)
  - 條件 > 取消勾選「只有在交流電源上才執行工作」(順便確認有沒有取消勾選「如果電腦切換到電池電源，則停止」) / 勾選 「喚醒電腦以執行工作」
  - 補償錯過的排程
    - 設定 > 勾選 「錯過排程的開始時間後盡快執行工作」
